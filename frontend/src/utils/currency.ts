export type CurrencyInfo = [
    ...
    string,  // Currency name (singular)
    string,  // Currency name (plural)
        number | null,  // ISO code or null if not available
    string,  // Symbol for the currency
    string,  // Alternate symbol or abbreviation
    string,  // Subunit name (singular)
    string,  // Subunit name (plural)
    number,  // Fractional digits (ISO standard)
    number,  // Fractional digits (currency standard)
        number | null  // Conversion factor to smallest unit or null if not applicable
];
 ...
 * @param currency - The currency code (ISO code) from the CURRENCY_LIST.
 * @returns A formatted string representing the value in the specified currency.
 */
export function formatCurrency(value: number, currency: string): string {
    const currencyInfo = CURRENCY_LIST[currency];

    if (!currencyInfo) {
        throw new Error(`Currency code ${currency} not found in CURRENCY_LIST.`);
    }

    const [
        , // Country or region (unused)
        , // Currency name singular (unused)
        , // Currency name plural (unused)
        , // ISO code (unused)
        symbol,
        , // Alternate symbol (unused)
        , // Subunit name singular (unused)
        , // Subunit name plural (unused)
        fractionalDigitsISO,
        fractionalDigitsCurrency,
        conversionFactor
    ] = currencyInfo;

    const fractionalDigits = fractionalDigitsISO !== null ? fractionalDigitsISO : fractionalDigitsCurrency;

    const formattedValue = (100 * (value / (conversionFactor || 1))).toFixed(fractionalDigits);
...
export const CURRENCY_KEYS: string[] = Object.keys(CURRENCY_LIST);

/**
 * Maps a currency code to a human-readable string.
 */
export type CurrencyMap = {
    [key: string]: string;
};

export const CURRENCY_MAP: CurrencyMap = Object.keys(CURRENCY_LIST).reduce((map, code) => {
    const [country, singular, , , symbol] = CURRENCY_LIST[code];
    const key = `${country} ${singular} (${symbol})`;
    map[key] = code;
    return map;
}, {} as CurrencyMap);

export const CURRENCY_NAMES: string[] = Object.keys(CURRENCY_MAP);

export function getFormattedCurrencyName(code: string): string {
    const [country, singular, , , symbol] = CURRENCY_LIST[code];
    return `${country} ${singular} (${symbol})`;
}
