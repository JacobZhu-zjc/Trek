import {Anchor, Box, Container, Group, Overlay, Stack, Text, Title} from '@mantine/core';
        ...
        <Box className={classes.wrapper}
             style={{
                 backgroundImage: (main_photo && typeof main_photo === "object" && main_photo.sizes?.tablet?.url)
                     ? `url('${uri}/${main_photo.sizes.tablet.url}')` : ''
             }}
             pt={100}
             w={"100vw"}>
            ...
            <Group justify="right" pt={100}>
                {(main_photo && typeof main_photo === "object" && main_photo.sizes?.tablet?.url) ?
                    (<Text c={"dimmed"} className='z-10'>
                        <Anchor c={"dimmed"}
                                href={main_photo.license_url || ''}>{main_photo.license?.toUpperCase()}</Anchor>
                        - {main_photo.artist}
                    </Text>) :
                    <></>}
            </Group>
