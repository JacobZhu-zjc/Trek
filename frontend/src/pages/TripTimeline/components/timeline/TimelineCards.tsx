...
export const TimelineCards = ({ timelineData }: TimelineCardsProps) => {
    ...
    // const renderTransportationItem = (item: TransportationItem, index: number) => (
    //     <>
    //         <Text>{item.startTime?.toLocaleTimeString('en-US', dateOptions)}</Text>
    //         <Draggable key={item.id} index={index} isDragDisabled={true} draggableId={item.id.toString()}>
    //             {(provided, snapshot) => (
    //                 <div
    ...
    //                         <Text>{getTextFromTransportationType(item.type, item.duration)}</Text>
    //                         <Text c="dimmed" size="sm">
    //                             Cost: {item.cost?.amount} {item.cost?.currency} • Duration: {item.duration?.humanize()}
    //                         </Text>
    //                     </div>
    //                 </div>
    //             )}
    ...
    const items = timelineData.map((item: TimelineItem, index) => (
        ...
        // item is a DestinationItem, render a destination item using renderDestinationItem()
        // item is a TransportationItem, render a transportation item using renderTransportationItem()
