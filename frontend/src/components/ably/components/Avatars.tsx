...
const SelfAvatar = ({self}: { self: Member | null }) => {
    ...
    return (
        <div
            ...
            style={{'--bg-color': self?.profileData.memberColor ?? "#EA580CFF"} as CSSProperties}
    ...
    return (
        <>
            {users.map((user, index) => {
                ...
                const initialsCSS: string = `${user.isConnected ? styles.textWhite : styles.inactiveColor} ${styles.nameOthers}`;

                const statusIndicatorCSS: string = `${user.isConnected ? styles.statusIndicatorOnline : styles.inactiveBackground} ${styles.statusIndicator}`;
